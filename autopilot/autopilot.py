...
from . import policy
from .autopilot_utils import default_settings, build_prompt, load_state, save_state
...
class AutoPilot:
    def __init__(self, tick_every_seconds=30, state_dir=None):
        ...
        self.next_tick_s = time.time() + tick_every_seconds
        os.makedirs(self.state_dir, exist_ok=True)


    def load_state(self, chat_id):
        return load_state(self.state_dir, chat_id)


    def save_state(self, chat_id, st):
        save_state(self.state_dir, chat_id, st)
    ...
    def format_status(self, chat_id):
        ...
        scheduled_ms = int(state.get("scheduled_send_ms", 0))
        scheduled_kind = state.get("scheduled_kind", "") or "—"
        reengage_ms = int(state.get("next_reengage_ms", 0))
        last_in_ms = int(state.get("last_inbound_ms", 0))
        last_out_ms = int(state.get("last_outbound_ms", 0))
        next_eligible_ms = int(state.get("next_eligible_send_ms", 0))
        pending = int(state.get("pending_inbound_count", 0))
        lines = [
            "AutoPilot status",
            f"Paused: {paused}",
        ...
        if reengage_ms:
            ...
            lines.append(f"Next re-engage target: {fmt_ms(reengage_ms)}"
                         + (f" (in ~{mins} min)" if mins is not None else ""))
        else:
    ...
    def tick(self, send_fn, generate_fn, introspection_fn=None):
        if time.time() < self.next_tick_s:
            return False
        self.next_tick_s = time.time() + self.tick_every_seconds
