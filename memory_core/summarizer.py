...
class Summarizer:
    ...
    # def weekly(self, llm, chat_id: int) -> str:
    #     """
    #     Legacy: picks last 7 daily summaries excluding the latest one.
    #     Kept so old calls don't break, but new scheduler should use weekly_range().
    #     """
    #     d = file_index.list_daily_summaries(self.paths.daily_summary_dir)
    #     if len(d) < 8:
    #         return ""
    #
    #     batch = d[:-1][:7]
    ...
    #         return ""
    #
    #     period = f"{batch[0].key} → {batch[-1].key}"
    #
    #     out_key = self.t.local_dt().isocalendar()
    #     yyyy = out_key[0]
    #     ww = out_key[1]
    #     out_name = f"{yyyy:04d}-{ww:02d}.md"
    #     out_path = os.path.join(self.paths.weekly_summary_dir, out_name)
    #
    #     if self.exists(out_path):
    #         return ""
    #
    #     src = "\n\n".join(file_index.read_text(x.path).strip() for x in batch if x.path)
    #     prompt = self.build_prompt(period=period, source_text=src)
    #     s = self.llm_summary_text(llm, chat_id, prompt)
    #     if not s:
    #         return ""
    ...
    #     return out_path
    #
    # def monthly(self, llm, chat_id: int) -> str:
    #     """
    #     Legacy: picks last 4 weekly summaries excluding the latest one.
    #     Kept so old calls don't break, but new scheduler should use monthly_range().
    #     """
    #     w = file_index.list_weekly_summaries(self.paths.weekly_summary_dir)
    #     if len(w) < 5:
    #         return ""
    #
    #     batch = w[:-1][:4]
    ...
    #     period = f"{batch[0].key} → {batch[-1].key}"
    #     dt = self.t.local_dt()
    #     out_name = f"{dt.year:04d}-{dt.month:02d}.md"
    #     out_path = os.path.join(self.paths.monthly_summary_dir, out_name)
    #
    #     if self.exists(out_path):
    #         return ""
    #
    ...
    #     return out_path
    #
    # def yearly(self, llm, chat_id: int) -> str:
    #     """
    #     Legacy: picks last 12 monthly summaries excluding the latest one.
    #     Kept so old calls don't break, but new scheduler should use yearly_range().
    #     """
    #     m = file_index.list_monthly_summaries(self.paths.monthly_summary_dir)
    #     if len(m) < 13:
    #         return ""
    #
    #     batch = m[:-1][:12]
    ...
    #     period = f"{batch[0].key} → {batch[-1].key}"
    #     dt = self.t.local_dt()
    #     out_name = f"{dt.year:04d}.md"
    #     out_path = os.path.join(self.paths.yearly_summary_dir, out_name)
    #
    #     if self.exists(out_path):
    #         return ""
    #
