...
class ContextBuilder:
    ...
    def update_low_priority(self, path) -> None:
        ...
        if weather_txt and weather_txt:
            helpers.write_text(path, weather_txt)
        low_text = helpers.read_text(path)
    ...
    def update_levels(self, st: dict) -> None:
        def join_files(paths_list):
            ...
            for p in paths_list or []:
                t = helpers.read_text(p)
                if t:
                    out.append(t)
            return "\n\n".join(out)

        master_txt = helpers.read_text(self.paths.master_path())
